...
local function CreateInteractionMenu(targetPlayer)
    ...
    frame.Paint = function(self, w, h)
        ...
        if rank == "vip" or rank == "vip+" or rank == "superadmin" then
            local rankText = targetPlayer:GetUserGroup()
            local x = w / 2 - surface.GetTextSize(rankText) / 2
            DrawRainbowText(rankText, "DermaLarge", x, 5, TEXT_ALIGN_CENTER)
        else
        ...
        draw.SimpleText("Money: $" .. money, "DermaDefault", 10, h - 80, Color(255, 255, 255), TEXT_ALIGN_LEFT)
